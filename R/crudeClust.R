...
crudeClust <- function( fF, channel, threshold, do.plot ){
  ...
  fF.high <- flowCore::Subset(fF, as.logical(flowCore::exprs(fF[, channel]) > threshold))
  ...
  if(do.plot){
    ...
    ggplot2::ggsave(filename = paste(filename, "_crude_clusters.pdf", sep=""),
                    plot=plt, width = 4, height = 4, units = "in")
    # grDevices::dev.off()
